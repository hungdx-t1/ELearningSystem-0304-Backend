...
public class SubmissionService : ISubmissionService
{
    ...
    public async Task<IEnumerable<SubmissionResponseDto>> GetSubmissionsByAssignmentIdAsync(Guid assignmentId)
    {
        var submissions = await _submissionRepo.FindAsync(s => s.AssignmentId == assignmentId);
        return submissions.Select(s => new SubmissionResponseDto(
            s.Id, s.AssignmentId, s.StudentId, s.SubmissionUrl, s.StudentNote, s.SubmittedAt, s.Score, s.Feedback));
    }

    public async Task<SubmissionResponseDto?> GetSubmissionAsync(Guid assignmentId, Guid studentId)
    {
        var subs = await _submissionRepo.FindAsync(s => s.AssignmentId == assignmentId && s.StudentId == studentId);
        var s = subs.FirstOrDefault();
        if (s == null) return null;
        
        return new SubmissionResponseDto(s.Id, s.AssignmentId, s.StudentId, s.SubmissionUrl, s.StudentNote, s.SubmittedAt, s.Score, s.Feedback);
    }
    ...
    public async Task<SubmissionResponseDto> SubmitWorkAsync(CreateSubmissionRequestDto request)
    {
        ...
        var existingSubs = await _submissionRepo.FindAsync(s => s.AssignmentId == request.AssignmentId && s.StudentId == request.StudentId);
        ...
        if (existingSub != null)
        {
            ...
            return new SubmissionResponseDto(existingSub.Id, existingSub.AssignmentId, existingSub.StudentId, existingSub.SubmissionUrl, existingSub.StudentNote, existingSub.SubmittedAt, existingSub.Score, existingSub.Feedback);
        }
        ...
        return new SubmissionResponseDto(newSub.Id, newSub.AssignmentId, newSub.StudentId, newSub.SubmissionUrl, newSub.StudentNote, newSub.SubmittedAt, newSub.Score, newSub.Feedback);
    }
